...
export default function Loader({ percent, stageText }) {
  ...
  useEffect(() => {
    if (percentRef.current) {
      gsap.fromTo(percentRef.current, { scale: 0.95, opacity: 0.8 }, { scale: 1.05, opacity: 1, duration: 0.3 });
    }
  }, [percent]);
    ...
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-[var(--almost-black)] text-white z-50">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center">
        <svg width={180} height={180} className="mb-6">
          <circle cx="90" cy="90" r={R} stroke="#222" strokeWidth={strokeWidth} fill="none" />
          <circle
            cx="90"
            cy="90"
            r={R}
        ...
        <div ref={percentRef} className="text-5xl font-extrabold" style={{ textShadow: '0 0 10px #fffb3b, 0 0 20px #fffb3b' }}>
          {percent}%
        </div>
